...
class SelfHostedGlobalSplitInstallManager implements GlobalSplitInstallManager {
    ...
    SelfHostedGlobalSplitInstallManager(SplitInstallManager delegate) {
    ...
    public void registerListener(final GlobalSplitInstallUpdatedListener listener) {
        SplitInstallStateUpdatedListener installStateListener = new SplitInstallStateUpdatedListener() {
            ...
            public void onStateUpdate(SplitInstallSessionState installState) {
                listener.onStateUpdate(SelfHostedGlobalSplitInstallSessionStateMapper.toGlobalSplitInstallSessionState(installState));
            }
        };
    ...
    public boolean startConfirmationDialogForResult(GlobalSplitInstallSessionState sessionState, Activity activity, int requestCode) throws IntentSender.SendIntentException {
        return delegate.startConfirmationDialogForResult(SelfHostedGlobalSplitInstallSessionStateMapper.toInstallState(sessionState), activity, requestCode);
    }
    ...
    public GlobalSplitInstallTask<Integer> startInstall(GlobalSplitInstallRequest request) {
        Task<Integer> featureTask = delegate.startInstall(
                SelfHostedGlobalSplitInstallRequestMapper.toFeatureInstallRequest(request));
        return new SelfHostedGlobalSplitInstallTask<Integer, Integer>(featureTask);
    }
    ...
    public GlobalSplitInstallTask<Void> cancelInstall(int sessionId) {
        return new SelfHostedGlobalSplitInstallTask<Void, Void>(delegate.cancelInstall(sessionId));
    }
    ...
    public GlobalSplitInstallTask<Void> deferredUninstall(List<String> moduleNames) {
        return new SelfHostedGlobalSplitInstallTask<Void, Void>(delegate.deferredUninstall(moduleNames));
    }

    @Override
    public GlobalSplitInstallTask<Void> deferredLanguageInstall(List<Locale> languages) {
        return new SelfHostedGlobalSplitInstallTask<Void, Void>(delegate.deferredLanguageInstall(languages));
    }

    @Override
    public GlobalSplitInstallTask<Void> deferredLanguageUninstall(List<Locale> languages) {
        return new SelfHostedGlobalSplitInstallTask<Void, Void>(delegate.deferredLanguageUninstall(languages));
    }

    @Override
    public GlobalSplitInstallTask<GlobalSplitInstallSessionState> getSessionState(int sessionId) {
        return new SelfHostedGlobalSplitInstallTask<SplitInstallSessionState, GlobalSplitInstallSessionState>(
                delegate.getSessionState(sessionId),
                new SelfHostedGlobalSplitInstallResultMapper<SplitInstallSessionState, GlobalSplitInstallSessionState>() {
                    @Override
                    public GlobalSplitInstallSessionState map(SplitInstallSessionState from) {
                        return SelfHostedGlobalSplitInstallSessionStateMapper.toGlobalSplitInstallSessionState(from);
                    }
                });
    }
    ...
    public GlobalSplitInstallTask<List<GlobalSplitInstallSessionState>> getSessionStates() {
        return new SelfHostedGlobalSplitInstallTask<List<SplitInstallSessionState>, List<GlobalSplitInstallSessionState>>(
                delegate.getSessionStates(),
                new SelfHostedGlobalSplitInstallResultMapper<List<SplitInstallSessionState>, List<GlobalSplitInstallSessionState>>() {
                    @Override
                    public List<GlobalSplitInstallSessionState> map(List<SplitInstallSessionState> from) {
                        ...
                        for (SplitInstallSessionState installState : from) {
                            ret.add(SelfHostedGlobalSplitInstallSessionStateMapper.toGlobalSplitInstallSessionState(installState));
                        }
                        return ret;
                    }
                });
